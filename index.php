				...
				<div class="col">
					<div class="p-3  exprow text-center mb-5">
						...
						<form action="" method="post">
							<button name="myButton" value=<?=$res['category'] ?> style="background:none; color:white; margin-top:10px; margin-bottom:10px;"><?=$res['category'] ?></button>
						</form>
...
if (isset($_POST['myButton'])) {
    ...
    $cat_id = intval($filtervalues); 
    $query = mysqli_query($con, "SELECT * FROM posts WHERE cat_id = $cat_id");
    ...
    if ($query) {
        if (mysqli_num_rows($query) > 0) {
            ...
            echo "<h2>No results found for the selected category.</h2>";
        }
    } else {
        echo "<h2>Error executing the query: " . mysqli_error($con) . "</h2>";
    }
}
?>
